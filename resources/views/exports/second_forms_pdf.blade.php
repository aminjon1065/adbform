...
<table>
    ...
    @forelse($items as $it)
        ...
        <tr>
            ...
            <td>
                {{ $it->rayon }}<br>
                <span class="muted">{{ $it->jamoat }}@if($it->selo), {{ $it->selo }}@endif</span>
            </td>
            <td>{{ $it->accept ? 'Да' : 'Нет' }}</td>
            <td>
                <strong>{{ $it->farm_name }}</strong><br>
                <span class="muted">{{ $it->leader_full_name }}, {{ $it->leader_age }}</span>
            </td>
            <td>{{ $it->leader_phone }}</td>
            <td>{{ $it->farm_plot_ha }}</td>
            <td>{{ $it->agriculture_experience }}</td>
            <td>{{ $seeds }}</td>
            <td>{{ $seedlings }}</td>
            <td>
                {{ $it->equipment_choice_label }}
                @if($it->equipment_choice === 'other' && $it->equipment_other_text)
                    <br><span class="muted">{{ $it->equipment_other_text }}</span>
                @endif
            </td>
            <td>{{ $irrigation }}</td>
    ...
    @empty
        <tr><td colspan="17" style="text-align:center;">Нет данных</td></tr>
    @endforelse
    </tbody>
</table>
</body>
