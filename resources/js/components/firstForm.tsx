...
type GardenKey =
    | "apricot" | "apple" | "grape" | "almond" | "persimmon" | "berries" | "other";
...
type FormData = {
    ...
    full_name: string;
    age: string;     // держим строкой, ограничиваем вручную
    ...
    irrigation_sources: Array<"none" | "well" | "pump" | "canal">;
...
// Безопасно форматируем YYYY-MM-DD для UI (без смещения по TZ)
const ymdToDate = (ymd?: string | null) => {
...
// Константы опций
const VEG_OPTIONS: Array<[label: string, key: VegKey]> = [
    ["Помидор", "tomato"],
...
const GARDEN_OPTIONS: Array<[label: string, key: GardenKey]> = [
    ["Абрикос", "apricot"],
    ["Яблоня", "apple"],
    ["Виноград", "grape"],
    ["Миндаль", "almond"],
...
const IRRIGATION_OPTIONS: Array<[label: string, key: "none" | "well" | "pump" | "canal"]> = [
    ["Нет", "none"],
    ["Скважина", "well"],
    ["Насос", "pump"],
    ["Канал / река", "canal"],
...
const FirstForm: React.FC = () => {
    ...
    const toggleIrrigation = (key: "none" | "well" | "pump" | "canal", checked: boolean) => {
        setData(
            "irrigation_sources",
            checked
    ...
    const resetForm = () => {
        setData(d => ({
            ...
            storage_area_sqm: "",
            has_refrigerator: false,
            // accept оставляем как есть (true)
        }));
    };
    ...
    const onSubmit = (e: React.FormEvent) => {
        ...
        if (showVegetable && anyVegChecked) {
            for (const v of Object.values(data.veg)) {
                if (v.checked && !v.area) {
                    alert("Для выбранных культур в овощеводстве укажите площадь (га).");
                    return;
                }
            }
        }
        if (showGarden && anyGardenChecked) {
            for (const v of Object.values(data.garden)) {
                if (v.checked && !v.area) {
                    alert("Для выбранных культур в садоводстве укажите площадь (га).");
                    return;
                }
            }
        }
        ...
        const seeds = showVegetable
            ? Object.entries(data.veg).filter(([, v]) => v.checked).map(([key, v]) => ({ key, area: v.area }))
            : [];
        const seedlings = showGarden
            ? Object.entries(data.garden).filter(([, v]) => v.checked).map(([key, v]) => ({ key, area: v.area }))
            : [];
        ...
        const payload = {
            ...
            children_count: data.children_count ? Number(data.children_count) : 0,
            ...
            storage_area_sqm: data.has_storage
                ? (data.storage_area_sqm ? Number(data.storage_area_sqm) : 0)
                : null,
                ...
                <CardContent className="grid gap-6">
                        ...
                        <Popover open={openDate} onOpenChange={setOpenDate}>
                            ...
                            <PopoverContent className="w-auto overflow-hidden p-0" align="start">
                                <Calendar
                                    ...
                                    onSelect={(d) => {
                                        const iso = d ? d.toISOString().slice(0, 10) : null; // YYYY-MM-DD
                    ...
                    <div className="grid gap-3">
                        ...
                        <div className="md:flex gap-2 space-y-3">
                            <Input placeholder="Район" value={data.rayon} onChange={(e) => setData("rayon", e.target.value)} />
                            <Input placeholder="Джамоат" value={data.jamoat} onChange={(e) => setData("jamoat", e.target.value)} />
                            <Input placeholder="Село" value={data.selo ?? ""} onChange={(e) => setData("selo", e.target.value)} />
                        </div>
                    </div>
              ...
              Я, нижеподписавшаяся, подтверждаю свою добровольную готовность принять участие в данном опросе…
                    ...
                    <div className="grid gap-3">
                        ...
                        <div className="md:flex gap-2 space-y-3">
                            ...
                            <Input
                                ...
                                value={data.phone}
                                maxLength={9}
                                onInput={(e) => setData("phone", onlyDigits(e.currentTarget.value).slice(0, 9))}
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid gap-3">
                        ...
                        <div className="md:flex gap-2 space-y-3">
                            <Input
                                ...
                                onInput={(e) => setData("family_count", onlyDigits(e.currentTarget.value))}
                            />
                            <Input
                                type="number"
                                min={0}
                                ...
                                onInput={(e) => setData("children_count", onlyDigits(e.currentTarget.value))}
                            />
                            <Input
                                type="number"
                                min={0}
                                ...
                                onInput={(e) => setData("elderly_count", onlyDigits(e.currentTarget.value))}
                            />
                            <Input
                                type="number"
                                min={0}
                                ...
                                onInput={(e) => setData("able_count", onlyDigits(e.currentTarget.value))}
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid gap-3">
                        ...
                        <Input
                            ...
                            value={data.plot_ha}
                            onInput={(e) => setData("plot_ha", e.currentTarget.value.replace(/[^\d.,]/g, ""))}
                        />
                    </div>
                    ...
                    <div className="grid gap-3">
                        <Label>6. Если Вы выбираете направление «Овощеводство», отметьте нужные семена и укажите площадь (га):</Label>
                        {showVegetable && (
                            <>
                                <div className="grid md:grid-cols-2 gap-3">
                                    {VEG_OPTIONS.map(([label, key]) => (
                                        <div key={key} className="flex items-center gap-2">
                                            <Checkbox
                                                id={`veg_${key}`}
                                                checked={data.veg[key].checked}
                                                onCheckedChange={(c) => setVegItem(key, { checked: Boolean(c) })}
                                            />
                                            <Label htmlFor={`veg_${key}`} className="whitespace-nowrap">{label}</Label>
                                            <Input
                                                className="ml-auto"
                                                placeholder="га"
                                                value={data.veg[key].area}
                                                onChange={(e) => setVegItem(key, { area: e.target.value })}
                                                disabled={!data.veg[key].checked}
                                ...
                                <p className="text-sm text-muted-foreground">
                                    Примечание: В пакет входят сеялка, опрыскиватель, защитная спецодежда, овощеводческий инвентарь.
                                </p>
                            </>
                        )}
                    </div>
                    ...
                    <div className="grid gap-3">
                        <Label>7. Если Вы выбираете направление «Садоводство», отметьте нужные саженцы и укажите площадь (га):</Label>
                        {showGarden && (
                            <>
                                <div className="grid md:grid-cols-2 gap-3">
                                    {GARDEN_OPTIONS.map(([label, key]) => (
                                        <div key={key} className="flex items-center gap-2">
                                            <Checkbox
                                                id={`garden_${key}`}
                                                checked={data.garden[key].checked}
                                                onCheckedChange={(c) => setGardenItem(key, { checked: Boolean(c) })}
                                            />
                                            <Label htmlFor={`garden_${key}`} className="whitespace-nowrap">{label}</Label>
                                            <Input
                                                className="ml-auto"
                                                placeholder="га"
                                                value={data.garden[key].area}
                                                onChange={(e) => setGardenItem(key, { area: e.target.value })}
                                                disabled={!data.garden[key].checked}
                                                required={data.garden[key].checked}
                                            />
                                        </div>
                                    ))}
                                </div>
                                <p className="text-sm text-muted-foreground">
                                    Примечание: В пакет входят ручной опрыскиватель, защитная спецодежда, садовый инвентарь (набор).
                                </p>
                            </>
                        )}
                    </div>
                    ...
                    <div className="grid gap-3">
                        ...
                        {data.has_storage && (
                            <Input
                                ...
                                onInput={(e) => setData("storage_area_sqm", onlyDigits(e.currentTarget.value))}
                                required
                            />
                        )}
                    </div>
